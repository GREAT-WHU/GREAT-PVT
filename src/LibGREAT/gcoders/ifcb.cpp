    ...
    int t_ifcb::decode_data(char* buff, int sz, int& cnt, vector<string>& errmsg)
    {
        ...
        try
        {
            while ((tmpsize = t_gcoder::_getline(tmp, 0)) >= 0)
            {
                ...
                {
                    GREAT_WARN(std::string("t_ifcb::decode_data End of file ") + tmp);
                ...
                {
                    GREAT_WARN(std::string("t_ifcb::decode_data unknown ifcb-data message ") + tmp);
                    t_gcoder::_consume(tmpsize);
                    _mutex.unlock();
                    return -1;
                }
        ...
        {
            GREAT_ERROR("t_ifcb::decode_data throw exception");
            return -1;
        }
        _mutex.unlock();
        return consume;
