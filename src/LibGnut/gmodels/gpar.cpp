...
namespace gnut
{
    ...
    t_gparhead::t_gparhead(par_type type, string site, string sat) :
        type(type),
        site(site),
        sat(sat)
    {
    ...
    {
        if (this->type == Other.type && this->sat == Other.sat && this->site == Other.site)
        {
            return true;
        }
        else
    ...
    bool t_gparhead::operator<(const t_gparhead& Other) const
    {
        if (this->type < Other.type || (this->type == Other.type && this->site < Other.site) ||
    ...
    {
        if (parType == par.parType && beg == par.beg && end == par.end)
        {
            return true;
        }
        else
    ...
    bool t_gpar::operator<(const t_gpar& par) const
    {
        if ((parType < par.parType) || (parType == par.parType && prn < par.prn) ||
