...
namespace gnut
{
    ...
    LibGnut_LIBRARY_EXPORT int ell2xyz(const double* Ell, double* XYZ,
                                       bool degrees); // True/False:  LatLon in Degrees/Radians!

    /**@brief XYZ to Ellipsoidal coordinates */
    LibGnut_LIBRARY_EXPORT int xyz2ell(const double* XYZ, double* Ell,
                                       bool degrees); // True/False:  LatLon in Degrees/Radians!
    ...
    LibGnut_LIBRARY_EXPORT int neu2xyz(const double* Ell, const double* neu,
                                       double* xyz); // ! Radians only: Ell[0], Ell[1]
    ...
    LibGnut_LIBRARY_EXPORT int rao2xyz(const ColumnVector& pos, const ColumnVector& vel, const ColumnVector& rao, ColumnVector& xyz);

    /**@brief XYZ coordinates to Radial/Along-track/Out-of-plane (Satell-system) */
    LibGnut_LIBRARY_EXPORT int xyz2rao(const ColumnVector& pos, const ColumnVector& vel, ColumnVector& xyz, ColumnVector& rao);
    ...
    LibGnut_LIBRARY_EXPORT int xyz2ell(const t_gtriple& crd, t_gtriple& ell,
                                       bool degrees); // True/False:  LatLon in Degrees/Radians!
    ...
    LibGnut_LIBRARY_EXPORT void xyz2neu(t_gtriple& ell, t_gtriple& xyz,
                                        t_gtriple& neu); // Radians only: Ell[0], Ell[1]

    /**@brief Topocentric/Local (NEU) to XYZ coordinates */
    LibGnut_LIBRARY_EXPORT void neu2xyz(t_gtriple& ell, t_gtriple& neu,
                                        t_gtriple& xyz); // Radians only: Ell[0], Ell[1]
