...
namespace gnut
{
    ...
    int t_biasinex::_decode_block()
    {
        ...
        if (_block.find("BIAS/DESCRIPTION") != string::npos)
        {
            ...
            {
                GREAT_DEBUG("Read BIAS/SMP: " + cut_crlf(_line.substr(31)));
            }
            else if (_line.find(" PARAMETER_SPACING ") != string::npos)
            {
                GREAT_DEBUG("Read BIAS/SPC: " + cut_crlf(_line.substr(31)));
            }
            else if (_line.find(" DETERMINATION_METHOD ") != string::npos)
            {
                GREAT_DEBUG("Read BIAS/MTD: " + cut_crlf(_line.substr(31)));
            }
            else if (_line.find(" BIAS_MODE ") != string::npos)
            {
                GREAT_DEBUG("Read BIAS/MOD: " + cut_crlf(_line.substr(31)));
            }
            else if (_line.find(" TIME_SYSTEM ") != string::npos)
            {
                GREAT_DEBUG("Read BIAS/TSY: " + cut_crlf(_line.substr(31)));
            }
        }
